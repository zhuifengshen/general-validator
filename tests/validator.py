...
def validate(data, *validations):
    ...
    示例用法：
    # 默认非空校验 - 最简形式
    validate(response, "data.product.id", "data.product.name")
    
    # 带校验器的形式
    validate(response, "data.product.id > 0", "data.product.price >= 10.5")
    ...
    # 列表批量校验 - 通配符
    validate(response, "data.productList.*.id", "data.productList.*.name")
    
    # 嵌套列表校验
    validate(response, "data.productList.*.purchasePlan.*.id > 0")
...
def _validate_field_path(data, field_path, validator, expect_value):
    ...
    """
    # 特殊处理条件校验
    if validator == "conditional_validator" or validator == "conditional_validate" or validator == "conditional_check":
...
def _execute_validator(validator, check_value, expect_value, field_path):
    ...
    try:
        ...
        elif validator == "conditional_validator" or validator == "conditional_validate" or validator == "conditional_check":
            # 条件校验逻辑 - 复用现有的解析和校验逻辑
...
def validate_not_empty(data, *field_paths):
    """专门的非空校验 - 最常用场景"""
    return validate(data, *field_paths)


def validate_when(data, condition, then):
    """
    条件校验 - 当条件满足时执行then校验
    ...
    validate_when(data, "status == 'active'", "price > 0")
    
    # 当type为premium时，features字段不能为空
    validate_when(data, "type == 'premium'", "features")
    
    # 支持通配符 - 当所有产品状态为active时，价格都必须大于0
    validate_when(data, "products.*.status == 'active'", "products.*.price > 0")
    ...
    return validate(data, conditional_rule)


def validate_list(data_list, *field_names, **validators):
    """
    列表数据批量校验 - 简化版
    ...
    示例：
    # 默认非空校验
    validate_list(productList, "id", "name", "price")
    
    # 带校验器
    validate_list(productList, "name", id="> 0", price=">= 0")
    
    # 混合使用
    validate_list(productList, "name", "description", id="> 0", status="== 'active'")
    ...
    # 执行校验
    return validate(data_list, *rules)


def validate_nested(data, list_path, nested_field, *field_validations):
    """
    嵌套列表数据批量校验 - 简化版
    ...
    示例：
    # 默认非空校验
    validate_nested(response, "data.productList", "purchasePlan", "id", "name")
    
    # 带校验器
    validate_nested(response, "data.productList", "purchasePlan", "id > 0", "amount >= 100")
    """
...
class DataValidator:
    ...
    def validate(self):
        ...
        注意：日志输出级别可通过项目的 --log-level 参数控制
        """
        return validate(self.data, *self.rules)
...
# 为了向后兼容，提供旧名称的别名
check = validate
check_not_empty = validate_not_empty
check_when = validate_when
check_list = validate_list
check_nested = validate_nested
DataChecker = DataValidator
checker = validator